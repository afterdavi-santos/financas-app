...
export function AvisoCategoriaSemelhante({ sugestoes, onSelecionar }: Props) {
  ...
  return (
    <div className="rounded-md border-l-4 border-red-500 bg-red-50 px-3 py-2 text-sm text-red-900">
      <p>
        ...
        <div className="mt-1.5 flex flex-wrap gap-2">
          {sugestoes.slice(0, 2).map((s) => (
            <button
              ...
              className="rounded-md border border-red-500 bg-white px-2 py-1 text-xs font-medium text-red-800 hover:bg-red-100"
            >
              Usar "{s.nome}"
            </button>
          ))}
